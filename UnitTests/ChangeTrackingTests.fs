...
let ``Initial data load should be reflected in change tracking items`` () : unit  =
    let changeTracker = ChangeTracker.create()
    
    let changeTracker = changeTracker |> ChangeTracker.processChanges initialLoadItems
...
let ``Initial data load should trigger correct callbacks``() : unit =
    ...
    let changeTracker = { ChangeTracker.create() with 
                            ItemAddedCallback   = Some (fun _ _ -> itemAddedCallbackInvocationCount   <- itemAddedCallbackInvocationCount   + 1) 
                            ItemDeletedCallback = Some (fun _   -> itemDeletedCallbackInvocationCount <- itemDeletedCallbackInvocationCount + 1) }

    let _ = changeTracker |> ChangeTracker.processChanges initialLoadItems
...
let ``Added data Item should be reflected in Change Tracker``() : unit =
    ...
    let changeTracker = changeTracker |> ChangeTracker.processChanges [Add(ChangeVersion 1L, addItem.Id, addItem)]
...
let ``Added data Item should increment Change Tracker Version``() : unit =
    ...
    let changeTracker = changeTracker |> ChangeTracker.processChanges initialLoadItems
    let changeTracker = changeTracker |> ChangeTracker.processChanges [Add((ChangeVersion 11L), addedItem.Id, addedItem)]
...
let ``Adding Item should trigger correct callbacks``() : unit =
    ...
    let _ = changeTracker |> ChangeTracker.processChanges initialLoadItems
    let _ = changeTracker |> ChangeTracker.processChanges [Add((ChangeVersion 7L), addItem.Id, addItem)]
...
let ``Deleted item should get removed from Change Tracker``() : unit =
    ...
    let changeTracker = changeTracker |> ChangeTracker.processChanges [Delete (ChangeVersion 99L, item1.Id)]
...
let ``Deleting Item should trigger correct callbacks``() : unit =
    ...
    let _ = changeTracker |> ChangeTracker.processChanges initialLoadItems
    let _ = changeTracker |> ChangeTracker.processChanges [Delete((ChangeVersion 8L), item1.Id)]
...
let ``Updated item should get updated in Change Tracker``(): unit =
    ...
    let changeTracker = changeTracker |> ChangeTracker.processChanges [Update(ChangeVersion 1L, updatedItem.Id, updatedItem)]
    
    Assert.Equal(3, changeTracker.Items.Count)
    Assert.Contains(changeTracker.Items.Values, fun i -> i.Name = "Billy Bob")
    Assert.DoesNotContain(changeTracker.Items.Values, fun i -> i.Name = item1.Name)
...
let ``Updated item should update Change Tracker Version as necessary``() : unit =
    ...
    let changeTracker = changeTracker |> ChangeTracker.processChanges [Update((ChangeVersion 19L), updatedItem.Id, updatedItem)]
